...
class cms_model extends CI_Model {
	...
	function generate_menu($items = array(), $ref_id = 0){
		$open = '';
		
		if($ref_id != 0){
		$tree = '<ul class="kt-menu__subnav">';
		} else {
		$tree = '<ul class="kt-menu__nav ">';
		}
		
		for($i=0, $ni=count($items); $i < $ni; $i++){
			if($items[$i]['ref_id'] == $ref_id){
				$active = '';
				$bullet = '';
				
				if($this->uri->uri_string()===$items[$i]['url']){
					$active = 'kt-menu__item--active';
				}
				
				if($ref_id==0){
					$bullet = '<span class="kt-menu__link-icon"><i class="'.$items[$i]['icon'].'"></i></span>';
				} else {
					$bullet = '<i class="kt-menu__link-bullet kt-menu__link-bullet--line"><span></span></i>';
				}
				
				if($this->cek_menu_items($items[$i]['id'])>0){
					if($this->cek_menu_open($items[$i]['id'],$this->uri->uri_string())>0){
						$open = 'kt-menu__item--open kt-menu__item--here';
					}
					
					$tree .= '<li class="kt-menu__item kt-menu__item--submenu '.$open.'" aria-haspopup="true"  data-ktmenu-submenu-toggle="hover"><a href="javascript:;" class="kt-menu__link  kt-menu__toggle">'.$bullet.'<span class="kt-menu__link-text">'.$items[$i]['nama'].'</span><i class="kt-menu__ver-arrow la la-angle-right"></i></a><div class="kt-menu__submenu "><span class="kt-menu__arrow"></span>';
				} else {
					$tree .= '<li class="kt-menu__item '.$active.'" aria-haspopup="true"><a href="'.site_url($items[$i]['url']).'" class="kt-menu__link  kt-menu__toggle">'.$bullet.'<span class="kt-menu__link-text">'.$items[$i]['nama'].'</span></a>';
				}
				$tree .= $this->generate_menu($items, $items[$i]['id']);
				if($this->cek_menu_items($items[$i]['id'])>0){
				$tree .= '</div></li>';
				} else {
				$tree .= '</li>';
