...
class Cms_users_group extends CI_Controller {
	...
	{						
		$this->data['page_title'] = "Manajemen User Group";
		...
		$this->data['add_javascript'] = array(
			base_url($this->config->item('assets')['assets_scripts'])."/cms_users_group.js",
		);
		
		$this->load->view('layout/header', array('config' => $this->_config, 'data' => $this->data));
		$this->load->view('cms/cms_users_group', $this->data);
	...
	{		
		...
		foreach ($list as $list_item) {
			...
			$row[] = '<div class="btn-group btn-group-sm" role="group">
					<a class="btn btn-primary" href="javascript:void()" title="Menu" onclick="groups_menu('."'".$list_item->id."','".$list_item->name."'".')"><i class="la la-sitemap"></i></a>
					<a class="btn btn-warning" href="javascript:void()" title="Edit" onclick="data_edit('."'".$list_item->id."'".')"><i class="la la-edit"></i></a>
				  <a class="btn btn-danger" href="javascript:void()" title="Hapus" onclick="data_delete('."'".$list_item->id."','".$list_item->name."'".')"><i class="la la-times-circle"></i></a></div>';
	...
	{				
		...
		foreach ($list as $list_item) {
			...
			$row[] = $no;						
			$row[] = $list_item->nama;		
			$row[] = '<button class="btn btn-sm btn-danger" title="Hapus" onclick="data_delete_personil('."'".$list_item->id."_".$list_item->group_id."','".$list_item->nama."'".')"><i class="fa fa-times"></i></button>';
	...
	{				
		...
		foreach ($list as $list_item) {
			...
			$row[] = $no;						
			$row[] = $list_item->nama;	
			$row[] = '<button class="btn btn-sm btn-success" title="Tambah" onclick="data_add_personil('."'".$list_item->id."','".$id_user_groups."'".')"><i class="fa fa-plus"></i></button>';
	...
	public function menu_list()
	{
		...
		foreach($main as $main_item){
			...
			if($group_menu_id != 0){
				$row[] = '<button class="btn btn-sm btn-success" onclick="data_edit_status('."'".$main_item->id."','".$group_menu_id."'".')">On</button>';
			}else{
			...
			for($j=1; $j<=$sub_len; $j++){
				if($main_item->id == $sub[$j][1]->ref_id){
					...
					foreach($sub[$j] as $sub_item){
						...
						if($group_menu_id != 0){
							$row[] = '<button class="btn btn-sm btn-success" onclick="data_edit_status('."'".$sub_item->id."','".$group_menu_id."'".')">On</button>';
						}else{
