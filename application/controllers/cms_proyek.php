...
class Cms_proyek extends CI_Controller {
	...
	{																	
		$this->data['page_title'] = "Manajemen Kegiatan";
		...
		$this->data['add_javascript'] = array(
			base_url($this->config->item('assets')['assets_scripts'])."/cms_proyek.js",
		);
		
		$this->load->view('layout/header', array('config' => $this->_config, 'data' => $this->data));
		$this->load->view('cms/cms_proyek', $this->data);
	...
	{		
		...
		foreach($main as $main_item){
			...
			if($main_len > 1){
				if($i == 1){
					//urutan 1
					$str_aksi = $str_aksi.'<a class="btn btn-info" href="javascript:void()" title="Turun posisi" onclick="data_edit_posisi('."'".$main_item->id."','down'".')"><i class="fa fa-chevron-down"></i></a>';
				}elseif($i == $main_len){
					//urutan akhir
					$str_aksi = $str_aksi.'<a class="btn btn-info" href="javascript:void()" title="Naik posisi" onclick="data_edit_posisi('."'".$main_item->id."','up'".')"><i class="fa fa-chevron-up"></i></a>';
				}else{
					//urutan diantara
					$str_aksi = $str_aksi.'<a class="btn btn-info" href="javascript:void()" title="Naik posisi" onclick="data_edit_posisi('."'".$main_item->id."','up'".')"><i class="fa fa-chevron-up"></i></a>
					<a class="btn btn-info" href="javascript:void()" title="Turun posisi" onclick="data_edit_posisi('."'".$main_item->id."','down'".')"><i class="fa fa-chevron-down"></i></a>';
				}
			}
			$i++;
			
			$row[] = '<div class="btn-group btn-group-sm" role="group">'.$str_aksi.'
					<a class="btn btn-warning" href="javascript:void()" title="Edit" onclick="data_edit('."'".$main_item->id."'".')"><i class="la la-edit"></i></a>
				  <a class="btn btn-danger" href="javascript:void()" title="Hapus" onclick="data_delete('."'".$main_item->id."','".$main_item->singkatan."'".')"><i class="la la-times-circle"></i></a></div>';
				...
				if($main_item->id == $sub[$j][1]->ref_id){
					...
					foreach($sub[$j] as $sub_item){
						...
						if($sub_sub_len > 1){
							if($k == 1){
								//urutan 1
								$str_aksi = $str_aksi.'<a class="btn btn-warning" href="javascript:void()" title="Turun posisi" onclick="data_edit_posisi('."'".$sub_item->id."','down'".')"><i class="fa fa-chevron-down"></i></a>';
							}elseif($k == $sub_sub_len){
								//urutan akhir
								$str_aksi = $str_aksi.'<a class="btn btn-warning" href="javascript:void()" title="Naik posisi" onclick="data_edit_posisi('."'".$sub_item->id."','up'".')"><i class="fa fa-chevron-up"></i></a>';
							}else{
								//urutan diantara
								$str_aksi = $str_aksi.'<a class="btn btn-warning" href="javascript:void()" title="Naik posisi" onclick="data_edit_posisi('."'".$sub_item->id."','up'".')"><i class="fa fa-chevron-up"></i></a>
						...
						$k++;
						$row[] = '<div class="btn-group btn-group-sm" role="group">'.$str_aksi.'
								<a class="btn btn-warning" href="javascript:void()" title="Edit" onclick="data_edit('."'".$sub_item->id."'".')"><i class="la la-edit"></i></a>
							  <a class="btn btn-danger" href="javascript:void()" title="Hapus" onclick="data_delete('."'".$sub_item->id."','".$sub_item->singkatan."'".')"><i class="la la-times-circle"></i></a></div>';
