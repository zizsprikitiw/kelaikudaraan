...
class Pesawat_manufacturer extends CI_Controller {
	
	//Global variable
	var $table_name = 'pesawat_manufacturer';
	...
	{																
		$this->data['page_title'] = "Manajemen Manufacturer";
		...
		$this->data['add_javascript'] = array(
			base_url($this->config->item('assets')['assets_scripts'])."/pesawat_manufacturer.js",
		);
		
		$this->load->view('layout/header', array('config' => $this->_config, 'data' => $this->data));
		$this->load->view('pesawat_manufacturer', $this->data);
		$this->load->view('layout/footer', array('config' => $this->_config, 'data' => $this->data));				
	}				
	
	public function data_list()
	{		
		$datatable_name = "pesawat_manufacturer";//$this->table_name
	...
	{				
		...
		$this->form_validation->set_rules('nama', 'Manufacturer', 'trim|required|is_unique[' . $this->table_name . '.nama]'); 
