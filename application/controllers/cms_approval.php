...
class Cms_approval extends CI_Controller {
	...
	{																	
		$this->data['page_title'] = "Pengaturan Persetujuan Laporan";
		...
		$this->data['add_javascript'] = array(
			base_url($this->config->item('assets')['assets_scripts'])."/cms_approval.js",
		);
		
		$this->load->view('layout/header', array('config' => $this->_config, 'data' => $this->data));
		$this->load->view('cms/cms_approval', $this->data);
				...
				if($no != 1){
					...
					foreach($temp_data as $temp_data_item){
						...
						if($temp_data_len > 1){
							if($level == 1){
								//urutan 1
								$str_aksi = $str_aksi.'<a class="btn btn-'.$btn.'" href="javascript:void()" title="Turun posisi" onclick="data_edit_posisi('."'".$id."','down'".')"><i class="fa fa-chevron-down"></i></a>';
							}elseif($level == $temp_data_len){
								//urutan akhir
								$str_aksi = $str_aksi.'<a class="btn btn-'.$btn.'" href="javascript:void()" title="Naik posisi" onclick="data_edit_posisi('."'".$id."','up'".')"><i class="fa fa-chevron-up"></i></a>';
							}else{
								//urutan diantara
								$str_aksi = $str_aksi.'<a class="btn btn-'.$btn.'" href="javascript:void()" title="Naik posisi" onclick="data_edit_posisi('."'".$id."','up'".')"><i class="fa fa-chevron-up"></i></a>
						...
						$rowin[] = '<div class="btn-group btn-group-sm" role="group">'.$str_aksi.'
								<a class="btn btn-warning" href="javascript:void()" title="Edit" onclick="data_edit('."'".$id."'".')"><i class="la la-edit"></i></a>
							  <a class="btn btn-danger" href="javascript:void()" title="Hapus" onclick="data_delete('."'".$id."','".$temp_data_item[1]." oleh ".$temp_data_item[2]."'".')"><i class="la la-times-circle"></i></a></div>';
