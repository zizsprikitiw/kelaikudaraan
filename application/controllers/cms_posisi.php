...
class Cms_posisi extends CI_Controller {
	...
	{																	
		$this->data['page_title'] = "Manajemen Jabatan Kegiatan";
		...
		$this->data['add_javascript'] = array(
			base_url($this->config->item('assets')['assets_scripts'])."/cms_posisi.js",
		);
		
		$this->load->view('layout/header', array('config' => $this->_config, 'data' => $this->data));
		$this->load->view('cms/cms_posisi', $this->data);
	...
	{		
		...
		foreach ($list as $list_item) {
			...
			if($main_len > 1){
				if($i == 1){
					//urutan 1
					$str_aksi = $str_aksi.'<a class="btn btn-info" href="javascript:void()" title="Turun posisi" onclick="data_edit_posisi('."'".$list_item->id."','down'".')"><i class="fa fa-chevron-down"></i></a>';
				}elseif($i == $main_len){
					//urutan akhir
					$str_aksi = $str_aksi.'<a class="btn btn-info" href="javascript:void()" title="Naik posisi" onclick="data_edit_posisi('."'".$list_item->id."','up'".')"><i class="fa fa-chevron-up"></i></a>';
				}else{
					//urutan diantara
					$str_aksi = $str_aksi.'<a class="btn btn-info" href="javascript:void()" title="Naik posisi" onclick="data_edit_posisi('."'".$list_item->id."','up'".')"><i class="fa fa-chevron-up"></i></a>
			...
			$row[] = '<div class="btn-group btn-group-sm" role="group" align="right">'.$str_aksi.'
					<a class="btn btn-primary" href="javascript:void()" title="Menu" onclick="posisi_menu('."'".$list_item->id."','".$list_item->nama."'".')"><i class="la la-sitemap"></i></a>
	...
	public function menu_list()
	{
		...
		foreach($main as $main_item){
			...
			if($posisi_menu_id != 0){
				$row[] = '<button class="btn btn-sm btn-success" onclick="data_edit_status('."'".$main_item->id."','".$posisi_menu_id."'".')">On</button>';
			}else{
			...
			for($j=1; $j<=$sub_len; $j++){
				if($main_item->id == $sub[$j][1]->ref_id){
					...
					foreach($sub[$j] as $sub_item){
						...
						if($posisi_menu_id != 0){
							$row[] = '<button class="btn btn-sm btn-success" onclick="data_edit_status('."'".$sub_item->id."','".$posisi_menu_id."'".')">On</button>';
						}else{
