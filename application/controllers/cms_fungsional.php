...
class Cms_fungsional extends CI_Controller {
	
	//Global variable
	var $table_name = 'fungsional';
	...
	{																	
		$this->data['page_title'] = "Manajemen Fungsional";
		...
		$this->data['add_javascript'] = array(
			base_url($this->config->item('assets')['assets_scripts'])."/cms_fungsional.js",
		);
		
		$this->load->view('layout/header', array('config' => $this->_config, 'data' => $this->data));
		$this->load->view('cms/cms_fungsional', $this->data);
	...
	{		
		...
		foreach ($list as $list_item) {
			...
			//add html for action
			$row[] = '<div class="btn-group btn-group-sm" role="group"><a class="btn btn-primary" href="javascript:void()" title="Menu" onclick="fungsional_menu('."'".$list_item->id."','".$list_item->nama."'".')"><i class="la la-sitemap"></i></a>
					<a class="btn btn-xs btn-warning" href="javascript:void()" title="Edit" onclick="data_edit('."'".$list_item->id."'".')"><i class="la la-edit"></i></a>
				  <a class="btn btn-danger" href="javascript:void()" title="Hapus" onclick="data_delete('."'".$list_item->id."','".$list_item->nama."'".')"><i class="la la-times"></i></a></div>';
	...
	{					
		...
		$this->form_validation->set_rules('nama', 'Fungsional', 'trim|required|is_unique[' . $this->table_name . '.nama]');		
	...
	public function menu_list()
	{
		...
		$fungsional_menu = $this->cms_model->fungsional_menu($id_fungsional);										
		...
		foreach($main as $main_item){
			...
			$fungsional_menu_id = $this->cms_model->menu_in_group($fungsional_menu, $main_item->id);
			if($fungsional_menu_id != 0){
				$row[] = '<button class="btn btn-sm btn-success" onclick="data_edit_status('."'".$main_item->id."','".$fungsional_menu_id."'".')">On</button>';
			}else{
			...
			for($j=1; $j<=$sub_len; $j++){
				if($main_item->id == $sub[$j][1]->ref_id){
					...
					foreach($sub[$j] as $sub_item){
						...
						$fungsional_menu_id = $this->cms_model->menu_in_group($fungsional_menu, $sub_item->id);
						if($fungsional_menu_id != 0){
							$row[] = '<button class="btn btn-sm btn-success" onclick="data_edit_status('."'".$sub_item->id."','".$fungsional_menu_id."'".')">On</button>';
						}else{
	...
	public function data_edit_status()
	{
		...
		$id_fungsional_menu = $this->input->post('id_fungsional_menu');
		
		if($id_fungsional_menu == 0){
			//tambahkan pada authority
			//pass validation			
			$additional_data = array(				
				'functions_id' => $id_menu,
				'fungsional_id'  => $id_fungsional
			);
			
			$insert = $this->cms_model->save($additional_data, 'authority_fungsional');			
		}else{
			//delete item authority
			$this->cms_model->delete('authority_fungsional', array('id' => $id_fungsional_menu));				
		}
