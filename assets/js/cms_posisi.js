...
$(document).ready(function() {		
	...
	table = $('#table').DataTable({ 			
		...
		"ajax": {
			"url": base_url+'cms_posisi/data_list',
			"type": "POST"
		},
	...
	table_menu = $('#table_menus').DataTable({ 			
		...
		// Load data for the table's content from an Ajax source
		"ajax": {
			"url": base_url+'cms_posisi/menu_list/',
			"type": "POST",
			"data":function ( d ) {
					var item_tab = document.getElementById('id_posisi');
					d.id_posisi = item_tab.value;			//document.getElementById('id_tabs').value					
				}										
		},
...
function data_edit(id)
{
  ...
  $.ajax({			  		
		url : base_url+'cms_posisi/data_edit/' + id,
		type: "GET",
		dataType: "JSON",
		...
		{				   
			...
			$('[name="jenis_dokumen"]').val(data['list'].jenis_dokumen);																	
...
function data_save()
{
  ...
  if(save_method == 'add') 
  {
	  url = base_url+'cms_posisi/data_save_add';
  }
  else
  {
	url = base_url+'cms_posisi/data_save_edit';
  }
...
function data_delete(id, nama_id)
{
	if(id != ''){
		...
		$.ajax({
			url : base_url+'cms_posisi/data_delete',
			type: "POST",
...
function posisi_menu(posisi_id, posisi_name)
{
	document.getElementById('id_posisi').value = posisi_id;
	reload_table_menu(); 
	$('#modalMenus').modal('show'); // show bootstrap modal when complete loaded
	$('.modal-title2').text('Menu '+posisi_name); // Set Title to Bootstrap modal title					
}	

function data_edit_status(menu_id, posisi_menu_id)
{
	var form_data = {
			id_posisi: document.getElementById('id_posisi').value,
			id_menu: menu_id,
			id_posisi_menu: posisi_menu_id					
		};
		
	$.ajax({
			url : base_url+'cms_posisi/data_edit_status',
			type: "POST",
			dataType: "JSON",
			data: form_data,
			success: function(data)
...
function data_edit_posisi(id, pos)
{
	...
	$.ajax({
			url : base_url+'cms_posisi/data_edit_posisi',
			type: "POST",
			dataType: "JSON",
			data: form_data,
			success: function(data)
