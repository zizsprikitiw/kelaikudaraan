...
$(document).ready(function() {																
	table = $('#table').DataTable({ 			
		...
		"ajax": {
			"url": base_url+'cms_pusat/data_list',
			"type": "POST"
		},
...
function data_edit(id)
{
  ...
  $.ajax({			  		
		url : base_url+'cms_pusat/data_edit/' + id,
		type: "GET",
		dataType: "JSON",
...
function data_save()
{
  ...
  if(save_method == 'add') 
  {
	  url = base_url+'cms_pusat/data_save_add';
  }
  else
  {
	url = base_url+'cms_pusat/data_save_edit';
  }
...
function data_delete(id, nama_id)
{
	if(id != ''){
		...
		$.ajax({
			url : base_url+'cms_pusat/data_delete',
			type: "POST",
